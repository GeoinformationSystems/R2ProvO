...
prov <- function(expression) {
    ...
    pkg.env$rdf %>% rdflib::rdf_add(
        subject = paste0(pkg.env$namespaces["ex"], output_iri),
    ...
    pkg.env$rdf %>% rdflib::rdf_add(
        subject = paste0(pkg.env$namespaces["ex"], process_instance),
        predicate = paste0(pkg.env$namespaces["rdf"], "type"),
    ...
    pkg.env$rdf %>% rdflib::rdf_add(
        ...
        object = paste0(pkg.env$namespaces["ex"], process_instance)
    )
    ...
    for (i in 2:length(right_side)) {
        ...
        pkg.env$rdf %>% rdflib::rdf_add(
            subject = paste0(pkg.env$namespaces["ex"], current_input_iri),
            predicate = paste0(pkg.env$namespaces["rdf"], "type"),
            object = paste0(pkg.env$namespaces["prov"], "Entity")
        )
        pkg.env$rdf %>% rdflib::rdf_add(
        ...
        pkg.env$rdf %>% rdflib::rdf_add(
            ...
            object = paste0(pkg.env$namespaces["ex"], current_input_iri)
        )
        pkg.env$rdf %>% rdflib::rdf_add(
            subject = paste0(pkg.env$namespaces["ex"], output_iri),
